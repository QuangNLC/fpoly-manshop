...
public interface StatusDetailJPA extends JpaRepository<StatusDetail, Integer> {
    ...
    List<Object> getRP();
    @Query( "select  new com.example.ManShop.DTOS.RPResquestDTO(max(s.orderStatus.title) , s.orders.id  )from StatusDetail  s group by s.orders.id")
