...
public interface PromotionJPA extends JpaRepository<Promotions,Integer> {
    @Query("select p from Promotions  p where p.datebefor > :date and p.isactive = false and p.isauto =false ")
    List<Promotions> ListFromStartDate(Date date);

    @Query("select p from Promotions  p where p.datebefor < :date and p.isactive = True  ")
